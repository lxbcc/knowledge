  ...
  "node_modules/obsidian-daily-notes-interface/dist/main.js"(exports) {
    ...
    async function getTemplateInfo(template) {
      ...
      try {
        ...
        console.error(`无法读取每日笔记模板 '${templatePath}'`, err);
        new obsidian.Notice("无法读取每日笔记模板");
        return ["", null];
      }
    }
...
var themeSave = async (plugin, cssGithubRepository, newInstall) => {
  ...
  if (!themeCss) {
    toastMessage(
      plugin,
      "此仓库的根路径中没有theme.css或theme-beta.css文件，因此没有可安装的主题。"
    );
    return false;
  }
  ...
  if (!themeManifest) {
    toastMessage(
      plugin,
      "此仓库的根路径中没有manifest.json文件，因此无法安装主题。"
    );
    return false;
  }
  ...
  if (newInstall) {
    ...
    msg = `从 ${cssGithubRepository} 安装了 ${manifestInfo.name} 主题。 `;
    setTimeout(() => {
      plugin.app.customCss.setTheme(manifestInfo.name);
    }, 500);
  } else {
    msg = `从 ${cssGithubRepository} 更新了 ${manifestInfo.name} 主题。`;
  }
...
var themeDelete = (plugin, cssGithubRepository) => {
  ...
  void plugin.saveSettings();
  const msg = `已从BRAT主题列表中移除${cssGithubRepository}，并将不再进行更新。但是，主题文件仍存在于保险库中。要删除它们，请转到“设置”>“外观”并删除该主题。`;
...
var promotionalLinks = (containerEl, settingsTab = true) => {
  ...
  captionText.innerText = "了解我的工作请访问：";
...
var AddNewTheme = class extends import_obsidian4.Modal {
  ...
  async submitForm() {
    ...
    if (existBetaThemeinInList(this.plugin, scrubbedAddress)) {
      toastMessage(this.plugin, `此主题已在测试版列表中`, 10);
      return;
    }
  ...
  onOpen() {
    this.contentEl.createEl("h4", { text: "Github仓库中的beta主题：" });
    this.contentEl.createEl("form", {}, (formEl) => {
      ...
      formEl.createDiv("modal-button-container", (buttonContainerEl) => {
        buttonContainerEl.createEl("button", { attr: { type: "button" }, text: "取消" }).addEventListener("click", () => {
          this.close();
        });
...
var BratSettingsTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian5.Setting(containerEl).setName("安装后自动启用插件").setDesc(
    ...
    new import_obsidian5.Setting(containerEl).setName("启动时自动更新插件").setDesc(
    ...
    new import_obsidian5.Setting(containerEl).setName("启动时自动更新主题").setDesc(
    ...
    containerEl.createEl("h2", { text: "Beta插件列表" });
    ...
    containerEl.createEl("p");
    containerEl.createEl("span").createEl("b", { text: "注释: " });
    containerEl.createSpan({
      text: "这不会删除插件，您应该从“设置”中的“社区插件”选项卡中删除它。"
    });
    new import_obsidian5.Setting(containerEl).addButton((cb) => {
      cb.setButtonText("添加测试版插件");
      cb.onClick(() => {
    ...
    for (const bp of this.plugin.settings.pluginList) {
      ...
      new import_obsidian5.Setting(containerEl).setName(createLink(bp)).addButton((btn) => {
        btn.setIcon("cross");
        btn.setTooltip("删除此测试版插件");
        btn.onClick(() => {
          if (btn.buttonEl.textContent === "")
            btn.setButtonText("再点击一次以确认删除");
          else {
            const { buttonEl } = btn;
    ...
    new import_obsidian5.Setting(containerEl).addButton((cb) => {
      cb.setButtonText("添加带有冻结版本的Beta插件");
      cb.onClick(() => {
        this.plugin.app.setting.close();
    ...
    containerEl.createEl("h2", { text: "Beta主题列表" });
    new import_obsidian5.Setting(containerEl).addButton((cb) => {
      cb.setButtonText("添加测试版主题");
      cb.onClick(() => {
        this.plugin.app.setting.close();
    ...
    containerEl.createEl("h2", { text: "检查" });
    new import_obsidian5.Setting(containerEl).setName("启用通知").setDesc(
    ...
    new import_obsidian5.Setting(containerEl).setName("启用日志记录").setDesc("插件更新将被记录到日志文件中。").addToggle((cb) => {
    ...
    new import_obsidian5.Setting(this.containerEl).setName("BRAT日志文件位置").setDesc("日志将被保存到这个文件中。不要在文件名中添加.md后缀。").addSearch((cb) => {
      cb.setPlaceholder("示例: BRAT-log").setValue(this.plugin.settings.loggingPath).onChange(async (newFolder) => {
        this.plugin.settings.loggingPath = newFolder;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian5.Setting(containerEl).setName("启用详细日志记录").setDesc("在日志中获取更多信息。").addToggle((cb) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("调试模式").setDesc(
    ...
    new import_obsidian5.Setting(containerEl).setName("个人访问令牌").setDesc(
      "如果您需要访问私有存储库，请在此处输入个人访问令牌。"
    ).addText((text) => {
      var _a;
      text.setPlaceholder("输入您的个人访问令牌").setValue((_a = this.plugin.settings.personalAccessToken) != null ? _a : "").onChange(async (value) => {
...
var AddNewPluginModal = class extends import_obsidian6.Modal {
  ...
  onOpen() {
    this.contentEl.createEl("h4", { text: "测试版插件的Github仓库：" });
    this.contentEl.createEl("form", {}, (formEl) => {
      formEl.addClass("brat-modal");
      ...
      if (this.useFrozenVersion) {
        new import_obsidian6.Setting(formEl).addText((textEl) => {
          textEl.setPlaceholder("指定发布版本标签(例如：1.0.0)");
          textEl.onChange((value) => {
    ...
    if (!manifestJson) {
      if (reportIssues) {
        toastMessage(
          this.plugin,
          `${repositoryPath}
这似乎不是一个Obsidian插件，因为没有找到manifest.json文件。`,
          noticeTimeout
        );
        console.error(
...
仓库的根目录中不存在manifest.json或manifest-beta.json文件。无法安装此插件。`;
    ...
    if (!Object.hasOwn(primaryManifest, "version")) {
      const msg = `${repositoryPath}
仓库根目录中的manifest${usingBetaManifest ? "-beta" : ""}.json文件中没有版本号。无法安装此插件。`;
      await this.plugin.log(msg, true);
      toastMessage(this.plugin, msg, noticeTimeout);
      return false;
    }
...
此插件的manifest${usingBetaManifest ? "-beta" : ""}.json 文件表明，Obsidian应用程序的版本需要为 ${primaryManifest.minAppVersion}, 但当前安装的Obsidian版本为 ${import_obsidian7.apiVersion}。 

您需要更新Obsidian以使用此插件，或联系插件开发者以获取更多信息。`;
        await this.plugin.log(msg, true);
...
插件已向BRAT注册。`;
      ...
      if (localManifestJson.version !== primaryManifest.version) {
        ...
        if (releaseFiles === null) return false;
        if (seeIfUpdatedOnly) {
          const msg = `${primaryManifest.id} 有可用更新，从版本 ${localManifestJson.version} 更新到 ${primaryManifest.version}. `;
          await this.plugin.log(
            msg + `[发布信息](https://github.com/${repositoryPath}/releases/tag/${primaryManifest.version})`,
            true
          );
          toastMessage(this.plugin, msg, 30, () => {
            if (primaryManifest) {
...
Plugin has been updated from version ${localManifestJson.version} to ${primaryManifest.version}. `;
        ...
        toastMessage(this.plugin, `没有 ${repositoryPath} 的可用更新`, 3);
    }
    return true;
  }
  /**
...
${msg1}`, 3e4);
  ...
  deletePlugin(repositoryPath) {
    const msg = `已从BRAT插件列表中移除 ${repositoryPath}`;
    ...
    this.bratCommands = [
      ...
      },
      {
        ...
        callback: () => {
          ...
          gfs.display((results) => {
            const msg = `正在检查 ${results.info} 的更新...`;
            void this.plugin.log(msg, true);
...
插件重新加载中.....`,
      ...
      },
      {
        ...
        callback: () => {
          ...
          gfs.setSuggesterData(pluginList);
          gfs.display((results) => {
            void this.plugin.log(`${results.display} 插件已禁用`, false);
      ...
      },
      {
        ...
        callback: () => {
          ...
          gfs.setSuggesterData(pluginList);
          gfs.display((results) => {
            void this.plugin.log(`${results.display} 插件已开启`, false);
  ...
  constructor() {
    ...
    this.obsidianProtocolHandler = (params) => {
      if (!params.plugin && !params.theme) {
        toastMessage(this, `无法从URL定位仓库。`, 10);
        return;
      }
