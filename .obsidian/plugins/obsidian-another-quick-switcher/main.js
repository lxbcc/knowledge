...
var createDefaultHotkeys = () => ({
  main: {
    ...
    "清除输入": [{ modifiers: ["Mod"], key: "d" }],
    "替换输入": [{ modifiers: [], key: "Tab" }],
    open: [{ modifiers: [], key: "Enter" }],
    "在新标签页中打开": [{ modifiers: ["Mod"], key: "Enter" }],
    "在新的水平窗格中打开": [{ modifiers: ["Mod"], key: "-" }],
    "在新的垂直窗格中打开": [{ modifiers: ["Mod"], key: "i" }],
    "在新窗口中打开": [{ modifiers: ["Mod"], key: "o" }],
    "在弹出窗口中打开": [],
    "在新标签页中打开 in background": [{ modifiers: ["Alt"], key: "o" }],
    "在新标签页中全部打开": [{ modifiers: ["Mod", "Shift", "Alt"], key: "o" }],
    preview: [{ modifiers: ["Mod"], key: "," }],
    create: [{ modifiers: ["Shift"], key: "Enter" }],
    "在新标签页中创建": [{ modifiers: ["Mod", "Shift"], key: "Enter" }],
    "在新窗口中创建": [{ modifiers: ["Mod", "Shift"], key: "o" }],
    "在新弹出窗口中创建": [],
    "使用默认应用打开": [],
    "在系统资源管理器中显示": [],
    "在谷歌中打开": [{ modifiers: ["Mod"], key: "g" }],
    "打开第一个URL": [{ modifiers: ["Mod"], key: "]" }],
    "插入到编辑器": [{ modifiers: ["Alt"], key: "Enter" }],
    "插入到编辑器 in background": [],
    "全部插入到编辑器": [{ modifiers: ["Alt", "Shift"], key: "Enter" }],
    "显示反向链接": [{ modifiers: ["Mod"], key: "h" }],
    "显示链接": [{ modifiers: ["Mod"], key: "l" }],
    "显示所有结果": [{ modifiers: ["Shift", "Alt"], key: "a" }],
    "向前导航": [{ modifiers: ["Alt"], key: "ArrowRight" }],
    "向后导航": [{ modifiers: ["Alt"], key: "ArrowLeft" }],
...
var AnotherQuickSwitcherSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "另一个快速切换器 - 设置" });
  ...
  addGeneralSettings(containerEl) {
    new import_obsidian3.Setting(containerEl).setName("搜索延迟毫秒").setDesc("如果键盘操作缓慢，尝试增加该值").addSlider(
    ...
    new import_obsidian3.Setting(containerEl).setName("建议的最大数量").addSlider(
    ...
    new import_obsidian3.Setting(containerEl).setName("标准化重音/变音符号").addToggle((tc) => {
      tc.setValue(
    ...
    if (this.plugin.settings.normalizeAccentsAndDiacritics) {
      containerEl.createEl("div", {
        text: "! 如果启用，它比禁用时慢大约2到5倍",
        cls: "another-quick-switcher__settings__warning"
      });
    }
    new import_obsidian3.Setting(containerEl).setName("使用选择的文字作为默认输入查询").addToggle((tc) => {
      tc.setValue(
    ...
    new import_obsidian3.Setting(containerEl).setName("防止重复的标签页").setDesc(
      "如果文件已经作为一个标签页打开，则不会在新标签页中打开；相反，现有的标签页会被激活。此选项适用于三个命令：'在新标签页中打开'，'在后台的新标签页中打开'，以及'在新标签页中全部打开'."
    ).addToggle((tc) => {
  ...
  addAppearanceSettings(containerEl) {
    containerEl.createEl("h3", { text: "\u{1F441}外观" });
    new import_obsidian3.Setting(containerEl).setName("显示目录").addToggle((tc) => {
    ...
    if (this.plugin.settings.showDirectory) {
      new import_obsidian3.Setting(containerEl).setName("在新行显示目录").setClass("another-quick-switcher__settings__nested").addToggle((tc) => {
      ...
      new import_obsidian3.Setting(containerEl).setName("显示目录的完整路径").setClass("another-quick-switcher__settings__nested").addToggle((tc) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("当关键词匹配别名时显示别名为标题").setDesc(
      "当关键词匹配别名时，显示别名为标题."
    ).addToggle((tc) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("显示别名为标题.").addToggle((tc) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("仅显示现有文件").addToggle((tc) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("隐藏边距图标").addToggle((tc) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("隐藏快捷键指南").addToggle((tc) => {
  ...
  addHotKeysInDialogSettings(containerEl) {
    containerEl.createEl("h3", { text: "\u2328对话框中的热键" });
    new import_obsidian3.Setting(containerEl).setName(
      "使用 `alt 1\uFF5E9` 而不是 `ctrl/cmd 1\uFF5E9` 快速选择结果"
    ).addToggle((tc) => {
      tc.setValue(
    ...
    const addHotkeyItems = (dialogKey, div) => {
      ...
      const addHotKeyItem = (name, command) => {
        new import_obsidian3.Setting(div).setName(name).setClass("another-quick-switcher__settings__dialog-hotkey-item").addText((cb) => {
          ...
          var _a;
          const dialog = this.plugin.settings.hotkeys[dialogKey];
          return cb.setTooltip("启用时显示快捷键指南").setValue(!((_a = dialog[command][0]) == null ? void 0 : _a.hideHotkeyGuide)).onChange(async (showHotkeyGuide) => {
    ...
    const addHotkeysForDialog = (dialogKey, dialogName) => {
      ...
      li.createEl("ul").createEl("li", {
        text: "对于空格键，请设置值为'Space'."
      });
      const ul = createEl("ul");
      ul.createEl("li", {
        text: "'Ctrl a'表示按下Ctrl键和A键."
      });
      ul.createEl("li", {
        text: "在Windows上使用'Mod'代替'Ctrl'，在macOS上使用'Cmd'."
      });
      ul.append(li);
  ...
  addSearchSettings(containerEl) {
    containerEl.createEl("h3", { text: "\u{1F50D} 搜索命令" });
    ...
    new import_obsidian3.Setting(containerEl).setHeading().addButton((btn) => {
      btn.setButtonText("添加").setTooltip("添加新命令").setCta().setClass(
...
`.trim(),
    ...
    new import_obsidian3.Setting(containerEl).setName("重置所有搜索命令").setClass("another-quick-switcher__settings__danger").setDesc(
      "这意味着你的自定义命令将会被移除。如果你不小心重置了，可以通过立即关闭设置和Obsidian，然后重新启动Obsidian来恢复搜索命令."
    ).addToggle((cb) => {
      ...
      btn.setButtonText("重置").setTooltip("重置所有搜索命令!!").setDisabled(this.resetLock).onClick(() => {
  ...
  addSearchCommandSetting(containerEl, command) {
    ...
    new import_obsidian3.Setting(div).setClass("another-quick-switcher__settings__search-command__header").setHeading().addText((tc) => {
      const el = tc.setPlaceholder("命令名称").setValue(command.name).onChange(async (value) => {
        command.name = value;
      });
      el.inputEl.setAttribute("style", "text-align: left");
      return el;
    }).addExtraButton((btn) => {
      btn.setTooltip("删除命令 (!! 它将无法恢复 !!)").setIcon("trash-2").onClick(() => {
    ...
    new import_obsidian3.Setting(div).setName("搜索方式").setDesc("点击按钮来启用/禁用搜索目标").addButton((bc) => {
      const coloring = () => {
      ...
      bc.setButtonText("标签").setClass(buttonClass).onClick(async () => {
      ...
      bc.setButtonText("标题").setClass(buttonClass).onClick(async () => {
      ...
      bc.setButtonText("链接").setClass(buttonClass).onClick(async () => {
      ...
      bc.setButtonText("属性").setClass(buttonClass).onClick(async () => {
    ...
    if (command.searchBy.property) {
      new import_obsidian3.Setting(div).setName("要搜索的属性键").setDesc("可以指定多个条目，使用换行符分隔.").addTextArea((tc) => {
    ...
    new import_obsidian3.Setting(div).setName("搜索目标").addDropdown((dc) => {
      dc.addOptions(mirror([...searchTargetList])).setValue(command.searchTarget).onChange(async (value) => {
        command.searchTarget = value;
      });
    });
    new import_obsidian3.Setting(div).setName('允许对 "搜索目标" 进行模糊搜索').addToggle((cb) => {
    ...
    new import_obsidian3.Setting(div).setName("最小模糊匹配得分").setDesc(
      "仅显示得分高于特定得分的建议"
    ).addSlider(
    ...
    new import_obsidian3.Setting(div).setName("目标扩展名").setDesc(
      "如果设置了，只有扩展名相符的文件才会被建议。如果为空，所有文件都会被建议。可以使用逗号设置多个扩展名."
    ).addTextArea(
      (tc) => tc.setPlaceholder("(例如：md,png,canvas)").setValue(command.targetExtensions.join(",")).onChange(async (value) => {
        command.targetExtensions = smartCommaSplit(value);
      })
    );
    new import_obsidian3.Setting(div).setName("浮动").addToggle((cb) => {
      cb.setValue(command.floating).onChange(async (value) => {
        command.floating = value;
        this.display();
      });
    });
    new import_obsidian3.Setting(div).setName("显示前置事项").addToggle((cb) => {
    ...
    if (command.showFrontMatter) {
      new import_obsidian3.Setting(div).setName("排除前置事项键").setDesc("可以使用换行符设置多个模式.").addTextArea((tc) => {
    ...
    new import_obsidian3.Setting(div).setName("默认输入").setDesc("打开对话框时的默认输入字符串").addText(
      (tc) => tc.setValue(command.defaultInput).setPlaceholder("(例如：#todo )").onChange(async (value) => {
        command.defaultInput = value;
      })
    );
    new import_obsidian3.Setting(div).setName("恢复上次输入").setDesc(
      "如果启用，此选项将在所有启用了它的搜索中恢复上次输入."
    ).addToggle((tc) => {
      tc.setValue(command.restoreLastInput).onChange(async (value) => {
        command.restoreLastInput = value;
      });
    });
    new import_obsidian3.Setting(div).setName("命令前缀").setDesc(
      "例如，如果设置了 ':r '，则查询以 ':r ' 开头意味着按此命令进行搜索"
    ).addText(
      (tc) => tc.setValue(command.commandPrefix).setPlaceholder("(例如：:r )").onChange(async (value) => {
    ...
    new import_obsidian3.Setting(div).setName("排序优先级").setDesc(df).addTextArea((tc) => {
    ...
    new import_obsidian3.Setting(div).setName("包含前缀路径模式").setDesc(
      "如果设置了，只有路径以其中一个模式开头的文件才会被建议。可以使用换行符设置多个模式。<current_dir> 表示当前目录."
    ).addTextArea((tc) => {
      const el = tc.setPlaceholder("(例如：Notes/Private)").setValue(command.includePrefixPathPatterns.join("\n")).onChange(async (value) => {
    ...
    new import_obsidian3.Setting(div).setName("排除前缀路径模式").setDesc(
      "如果设置了，路径以其中一个模式开头的文件将不会被建议。可以使用换行符设置多个模式。<current_dir> 表示当前目录."
    ).addTextArea((tc) => {
      const el = tc.setPlaceholder("(例如：Notes/Private)").setValue(command.excludePrefixPathPatterns.join("\n")).onChange(async (value) => {
  ...
  addHeaderSearchSettings(containerEl) {
    containerEl.createEl("h3", { text: "\u{1F4D2} 标题搜索" });
    new import_obsidian3.Setting(containerEl).setName("浮动模式下的自动预览").addToggle((tc) => {
      tc.setValue(
  ...
  addBacklinkSettings(containerEl) {
    containerEl.createEl("h3", { text: "\u{1F50D} 反向链接搜索" });
    new import_obsidian3.Setting(containerEl).setName('排除前缀路径模式用于 "反向链接搜索"').setDesc(
      "如果设置了，路径以其中一个模式开头的文件夹将不会被建议。可以使用换行符设置多个模式"
    ).addTextArea((tc) => {
      const el = tc.setPlaceholder("前缀匹配模式").setValue(
  ...
  addInFileSettings(containerEl) {
    containerEl.createEl("h3", { text: "\u{1F50D} 文件内搜索" });
    new import_obsidian3.Setting(containerEl).setName("上下文行").setDesc(
      "指定在目标行前后显示的行数。例如，将其设置为 '2' 会在目标行前后各显示两行，为目标文本提供上下文"
    ).addSlider(
    ...
    new import_obsidian3.Setting(containerEl).setName("匹配词周围的最大显示长度").setDesc(
      "匹配词前后最大的显示字符数."
    ).addSlider(
  ...
  addGrepSettings(containerEl) {
    ...
    new import_obsidian3.Setting(containerEl).setName("Ripgrep命令").setDesc("可以执行ripgrep的命令").addText(
      (tc) => tc.setValue(this.plugin.settings.ripgrepCommand).onChange(async (value) => {
        this.plugin.settings.ripgrepCommand = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian3.Setting(containerEl).setName("扩展名").addText(
      (tc) => tc.setPlaceholder("(例如：md,html,css)").setValue(this.plugin.settings.grepExtensions.join(",")).onChange(async (value) => {
  ...
  addMoveSettings(containerEl) {
    containerEl.createEl("h3", { text: "\u{1F4C1} 将文件移动到另一个文件夹" });
    new import_obsidian3.Setting(containerEl).setName('排除前缀路径模式用于 "将文件移动到另一个文件夹"').setDesc(
      "如果设置了，路径以其中一个模式开头的文件夹将不会被建议。可以使用换行符设置多个模式"
    ).addTextArea((tc) => {
      const el = tc.setPlaceholder("前缀匹配模式").setValue(
  ...
  addDebugSettings(containerEl) {
    ...
    new import_obsidian3.Setting(containerEl).setName("在控制台显示性能记录").addToggle((tc) => {
      tc.setValue(
    ...
    new import_obsidian3.Setting(containerEl).setName("在对话框中显示模糊匹配得分").addToggle((tc) => {
...
var AnotherQuickSwitcherModal = class _AnotherQuickSwitcherModal extends import_obsidian4.SuggestModal {
  ...
  setHotkeys() {
    ...
    this.registerKeys("清除输入", () => {
      this.inputEl.value = "";
      this.inputEl.dispatchEvent(new Event("input"));
    });
    this.registerKeys("替换输入", () => {
    ...
    this.registerKeys("在谷歌中打开", () => {
      activeWindow.open(`https://www.google.com/search?q=${this.searchQuery}`);
      this.close();
    });
    this.registerKeys("打开第一个URL", async () => {
      var _a, _b;
    ...
    this.registerKeys("插入到编辑器", async () => {
      var _a, _b;
      const item = (_a = this.chooser.values) == null ? void 0 : _a[this.chooser.selectedItem];
      if (!item) {
        return;
    ...
    this.registerKeys("插入到编辑器 in background", async () => {
      var _a, _b;
      const item = (_a = this.chooser.values) == null ? void 0 : _a[this.chooser.selectedItem];
      if (!item) {
        return;
    ...
    this.registerKeys("全部插入到编辑器", async () => {
      var _a;
      await this.safeClose();
      let offsetX = 0;
...
var GrepModal = class extends import_obsidian7.SuggestModal {
  constructor(app, settings, initialLeaf) {
    ...
    if (searchCmd) {
      ...
      this.setPlaceholder(`通过${inst == null ? void 0 : inst.command}键在Vault中搜索`);
    } else {
      this.setPlaceholder(
        `请在"Grep对话框"设置中设置一个关于"搜索"的键`
      );
    }
    this.setHotkeys();
  }
...
var InFileModal = class extends import_obsidian9.SuggestModal {
  constructor(app, settings, initialLeaf) {
    ...
    this.limit = 255;
    this.setHotkeys();
    this.setPlaceholder("输入任意内容然后显示结果");
  }
  close() {
