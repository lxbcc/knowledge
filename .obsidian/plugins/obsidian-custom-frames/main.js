...
var CustomFramesSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    this.containerEl.createEl("p", {
      text: "请注意，对于这些设置中的大多数，需要重新启动或重新加载Obsidian才能生效",
      cls: "mod-warning"
    });
    new import_obsidian2.Setting(this.containerEl).setName("框架填充").setDesc("自定义框架窗格内部周围应留出的填充，以像素为单位。").addText((t) => {
    ...
    for (let frame of this.plugin.settings.frames) {
      ...
      content.hidden = true;
      new import_obsidian2.Setting(content).setName("显示名称").setDesc("此框架应有的显示名称。").addText((t) => {
      ...
      new import_obsidian2.Setting(content).setName("图标").setDesc(createFragment((f) => {
        f.createSpan({ text: "此框架窗格应具有的图标。可以使用任何 " });
        f.createEl("a", { text: "Lucide 图标", href: "https://lucide.dev/" });
        f.createSpan({ text: " 的名字。" });
      })).addText((t) => {
      ...
      new import_obsidian2.Setting(content).setName("URL").setDesc("应在该框架中打开的URL。").addText((t) => {
      ...
      new import_obsidian2.Setting(content).setName("在移动设备上禁用").setDesc("在移动设备上，自定义框架受到更多限制，并不允许显示相同类型的内容。如果一个框架在移动设备上无法按预期工作，可以将其禁用。").addToggle((t) => {
      ...
      new import_obsidian2.Setting(content).setName("添加带状图标的按钮").setDesc("是否应将用于打开此框架的按钮添加到带状栏。").addToggle((t) => {
      ...
      new import_obsidian2.Setting(content).setName("在中心打开").setDesc("此框架是否应在未固定的中心编辑器中打开而不是侧边的窗格之一。这对于那些在窄视图下表现不佳或查看时不需要打开笔记的网站很有用。").addToggle((t) => {
      ...
      new import_obsidian2.Setting(content).setName("强制使用iframe").setDesc(createFragment((f) => {
        f.createSpan({ text: "此框架在桌面上是否应使用iframe而非Electron网页视图。" });
        f.createEl("br");
        f.createEl("em", { text: "只有当框架导致问题或频繁崩溃时才启用此设置。此设置会导致所有仅限桌面的设置被忽略。"});
      })).addToggle((t) => {
      ...
      new import_obsidian2.Setting(content).setName("附加CSS").setDesc(createFragment((f) => {
        f.createSpan({ text: "一段应应用于此框架的附加CSS代码。" });
        f.createEl("br");
        f.createEl("em", { text: "请注意，这仅适用于桌面环境。"});
      })).addTextArea((t) => {
        t.inputEl.rows = 5;
        t.inputEl.cols = 50;
      ...
      new import_obsidian2.Setting(content).setName("附加JavaScript").setDesc(createFragment((f) => {
        f.createSpan({ text: "一段应应用于此框架的附加JavaScript代码。" });
        f.createEl("br");
        f.createEl("em", { text: "请注意，这仅适用于桌面环境。"});
      })).addTextArea((t) => {
        t.inputEl.rows = 5;
    ...
    this.containerEl.createEl("p", { text: `创建一个新的框架，可以从插件自带的预设中创建，或者创建一个您可以自己编辑的自定义框架。每个框架的窗格都可以通过“Custom Frames: Open”命令打开。`});
    ...
    disclaimer.createSpan({ text: "请注意，当您将一个网站添加为自定义框架时，您可能将输入的个人信息暴露给已安装的其他插件。更多信息，请参见 " });
    ...
    this.containerEl.createEl("hr");
    this.containerEl.createEl("p", { text: "如果您喜欢这个插件并希望支持其开发，您可以通过点击这个漂亮的图片在我的网站上进行。"});
